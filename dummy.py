...
class DataValidation:
    ...
    def __init__(self,data_validation_config:config_entity.DataValidationConfig,data_ingestion_artifact:artifact_entity.DataIngestionArtifact):
        try:
            logging.info(f"{'>>'*20} Data vaildation {'<<'*20} ")
            self.data_validation_config = data_validation_config
            self.data_ingestion_artifact=data_ingestion_artifact
            self.validation_error = dict()  #for generating a proper report by collecting info in below subsequent codes
    ...
    def drop_missing_columns(self,df:pd.DataFrame,report_key_name:str)-> Optional[pd.DataFrame]:
        ...
        try:
            ...
            #self.vaildation_error [report_key_name] = drop_column_names       this line will asloo work but the data in report will be non asthetic
    ...
    def data_drift(self,base_df:pd.DataFrame, current_df:pd.DataFrame,report_key_name:str):
        try:
            ...
            for base_column in base_columns:
                ...
                if same_distrubution>0.05:
                    #We are accepting null hypothesis
                    drift_report[base_column]={
                        "p_values":float(same_distrubution.pvalue),
